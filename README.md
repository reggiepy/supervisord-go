...
Alternatively, you can build the supervisord executable with stripped debug information for a smaller binary:

```bash
go build -ldflags="-s -w" -tags release -o supervisord.exe
```

### Explanation of Build Flags

- **`-tags release`**: This build tag enables code that is conditionally compiled for the `release` build.
- **`-ldflags="-s -w"`**: These linker flags remove debugging information:
  - **`-s`**: Omit the symbol table and debug information.
  - **`-w`**: Omit the DWARF debugging information.

The `supervisord.exe` file generated by these commands is the Windows executable for `supervisord`.
